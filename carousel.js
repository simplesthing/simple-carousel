...
/***/ function(module, exports, __webpack_require__) {
	...
	var place = uri.search ? decodeURI(uri.search.split('=')[1]) : 'Seattle';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var transition = 'transition: left 750ms ease;';
	...
	var Carousel = function () {
	  ...
	  _createClass(Carousel, [{
	    ...
	    value: function transitionComplete() {
	      ...
	      var newStyles;
	      var styles = carousel.getAttribute('style').split(';');
	    ...
	    key: 'items',
	    value: function items(photo, length) {
	    ...
	    key: 'build',
	    value: function build(photos, domElem) {
	      var _this = this;

	      var _photos = photos.concat(photos);
	      var template = this.template(photos.length);
	      var list = template.querySelector('#carousel_list');
	      var items = _photos.map(function (item) {
	        list.appendChild(_this.items(item));
	      });
	      var _carousel = document.getElementById(domElem);
	      _carousel.appendChild(template);
	      this.events();
	    }
	  }, {
	    key: 'init',
	    value: function init(place, domElem) {
	      var _this2 = this;

	      // instantiate a new API class to call Flickr
	      var API = new _api2.default();
	      // get place_id for Seattle
	      API.placeId(place).then(function (res) {
	        API.photos(res).then(function (results) {
	          _this2.length = results.photos.photo.length;
	          _this2.width = results.photos.photo.length * 200;
	          _this2.build(results.photos.photo, domElem);
	        });
	      });
	    }
	  }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Flicker = __webpack_require__(3);
	var flickr = new Flickr({ api_key: '4f909c5048c2d990084b01f981c302d9' });
